...
TEST_F(TouchableUpdaterTest, coincident)
{
    ...
    // Coincident point should work in either volume
    for (auto& xdir : {1.0, -1.0})
    {
        for (auto* lv : {tracker_lv, calo_lv})
        {
            EXPECT_TRUE(update({125, 0, 0}, {xdir, 0, 0}, lv));
        }
    }
...
TEST_F(TouchableUpdaterTest, just_outside_warn)
{
    ...
    static char const* const expected_log_messages[] = {
        ...
        "Bumping navigation state by 0.20000000000001 [mm] at {1250.2, 0, 0} "
        "[mm] along {-1, 0, 0} from {{pv='em_calorimeter_pv', "
...
TEST_F(TouchableUpdaterTest, too_far)
{
    ...
    for (auto& xdir : {1.0, -1.0})
    {
        ...
        EXPECT_FALSE(update({125 + 2 * eps, 0, 0}, {-xdir, 0, 0}, tracker_lv));
    }
    ...
    static char const* const expected_log_messages[] = {
        ...
        "Failed to bump navigation state up to a distance of 1 [mm] at {1270, "
        "0, 0} [mm] along {-1, 0, 0} to try to reach \"si_tracker\"@0x0 "
