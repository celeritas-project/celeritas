...
class GeantGdmlLoaderTest : public ::celeritas::test::Test
{
  protected:
    ...
    size_type count(MapDetectors const& detectors, std::string_view name)
    {
        return std::count_if(
            detectors.begin(), detectors.end(), [&name](auto const& kv) {
                return kv.first.first == name;
            });
    }
...
TEST_F(SimpleCmsTest, detectors)
{
    ...
    EXPECT_EQ(1, this->count(loaded.detectors, "si_tracker_sd"));
    EXPECT_EQ(1, this->count(loaded.detectors, "em_calorimeter_sd"));
...
TEST_F(CmsEeTest, ignore)
{
    ...
    EXPECT_EQ(2, this->count(loaded.detectors, "ee_back_plate"));
    EXPECT_EQ(2, this->count(loaded.detectors, "ee_s_ring"));
