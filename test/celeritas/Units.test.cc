...
TEST(UnitsTest, clhep)
{
#if CELERITAS_USE_GEANT4
    {
        ...
        auto celer_native = convert_from_geant<units::ClhepField>(g4_native);
        ...
        EXPECT_SOFT_EQ(g4_native,
                       convert_to_geant<units::ClhepField>(celer_native));
    }
    {
        double g4_native = 1.5 * CLHEP::s;
        auto celer_native = convert_from_geant<units::ClhepTime>(g4_native);
        EXPECT_SOFT_EQ(1.5_s, celer_native);
        EXPECT_SOFT_EQ(g4_native,
                       convert_to_geant<units::ClhepTime>(celer_native));
    }
    {
