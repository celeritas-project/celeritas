...
ImportPhysicsVector mock_vec(unsigned int i, unsigned int j)
{
    ...
    return ImportPhysicsVector{ImportPhysicsVectorType::linear, grids[i], values[j]};
}
...
std::vector<ImportOpticalModel> const& MockImportedData::import_models()
{
    ...
    static std::vector<ImportOpticalModel> models{
        {IMC::absorption, {mock_vec(0, 0), mock_vec(1, 1), mock_vec(1, 2), mock_vec(2, 3), mock_vec(3, 4)}},
        {IMC::rayleigh, {mock_vec(1, 0), mock_vec(0, 1), mock_vec(3, 3), mock_vec(2, 3), mock_vec(0, 2)}},
        {IMC::wls, {mock_vec(3, 4), mock_vec(1, 0), mock_vec(1, 1), mock_vec(2, 4), mock_vec(3, 4)}},
        {IMC::other, {mock_vec(0, 0), mock_vec(1, 0), mock_vec(2, 4), mock_vec(3, 3), mock_vec(1, 0)}},
    };
...
{
    static std::vector<ImportOpticalMaterial> materials(5, ImportOpticalMaterial());
    return materials;
}
 ...
 */
auto MockImportedData::absorption_id() -> ImportedModelId { return ImportedModelId{0}; }

//---------------------------------------------------------------------------//
/*!
 * Imported model ID for Rayleigh scattering.
 */
auto MockImportedData::rayleigh_id() -> ImportedModelId { return ImportedModelId{1}; }

//---------------------------------------------------------------------------//
/*!
 * Imported model ID for wavelength shifting.
 */
auto MockImportedData::wls_id() -> ImportedModelId { return ImportedModelId{2}; }
...
auto MockImportedData::create_empty_imported_models() const -> SPConstImported
{
    ...
    ImportPhysicsVector const empty_vec{ImportPhysicsVectorType::linear, {}, {}};
    for (auto const& model : this->import_models())
    {
        empty_models.push_back({model.model_class, ImportedMfpTable(model.mfps.size(), empty_vec)});
    }
 ...
 */
void MockImportedData::check_mfp(ImportPhysicsVector const& expected, ImportPhysicsVector const& imported) const
{
 ...
 */
void MockImportedData::check_built_table(ImportedMfpTable const& expected_mfps, ItemRange<Grid> const& table) const
{
