...
inp::Grid native_grid_from(std::vector<double> xs, std::vector<double> ys)
{
...
void OpticalMockTestBase::build_import_data(ImportData& data) const
{
    ...
    auto mfp = [&](std::vector<double> x, std::vector<double> y) {
        return native_grid_from<units::Mev, units::Centimeter>(std::move(x),
                                                               std::move(y));
    };
    ...
    mat_props(0).refractive_index
        = native_grid_from<units::ElectronVolt, units::Native>(
    ...
    model(0, bulk.wls).mfp = mfp({1e-3, 2e-3, 5e-1}, {1.3, 4.9, 9.4});
    ...
    model(0, bulk.mie).mfp = mfp({1e-1, 1e1}, {2.3, 5.4});
    model(0, bulk.absorption).mfp = mfp({1e-3, 1e-2}, {5.7, 9.3});
    model(0, bulk.rayleigh).mfp = mfp({1e-2, 3e2}, {5.7, 9.3});
    ...
    mat_props(1).refractive_index
        = native_grid_from<units::ElectronVolt, units::Native>(
            {1.098177, 1.256172, 1.484130},
            {1.3235601610672, 1.3256740639273, 1.3280120256415});
    model(1, bulk.absorption).mfp = mfp({1e-2, 3e2}, {1.2, 10.7});
    model(1, bulk.rayleigh).mfp = mfp({1e-3, 1e-2}, {1.2, 10.7});
    model(1, bulk.wls).mfp = mfp({1e-2, 3e2}, {5.7, 9.3});
    model(1, bulk.wls2).mfp = mfp({2e-2, 1e0, 3e2}, {5.7, 6.2, 9.3});
    model(1, bulk.mie).mfp = mfp({2e-2, 1e0, 3e2}, {5.7, 6.2, 9.3});

    // Material 2
    mat_props(2).refractive_index
        = native_grid_from<units::ElectronVolt, units::Native>(
            {1.098177, 6.812319}, {1.3235601610672, 1.4679465862259});
    model(2, bulk.absorption).mfp = mfp({1e-2, 3e2}, {3.1, 5.4});
    model(2, bulk.rayleigh).mfp = mfp({1e-3, 2e-3, 5e-1}, {0.1, 7.6, 12.5});
    model(2, bulk.wls).mfp = mfp({1e-2, 3e2}, {1.2, 10.7});
    model(2, bulk.wls2).mfp = mfp({3e-2, 3e2}, {3.2, 9.4});
    model(2, bulk.mie).mfp = mfp({3e-2, 3e2}, {3.2, 9.4});

    // Material 3
    mat_props(3).refractive_index
        = native_grid_from<units::ElectronVolt, units::Native>(
            {1, 2, 5}, {1.3, 1.4, 1.5});
    model(3, bulk.absorption).mfp = mfp({2e-3, 5e1, 1e2}, {0.1, 7.6, 12.5});
    model(3, bulk.rayleigh).mfp = mfp({2e-3, 5e1, 1e2}, {0.1, 7.6, 12.5});
    model(3, bulk.wls).mfp = mfp({2e-3, 5e1, 1e2}, {1.3, 4.9, 9.4});
    model(3, bulk.wls2).mfp = mfp({2e-3, 2e2}, {4.9, 9.4});
    model(3, bulk.mie).mfp = mfp({2e-3, 2e2}, {4.9, 9.4});

    // Material 3
    mat_props(4).refractive_index
        = native_grid_from<units::ElectronVolt, units::Native>(
            {1.098177, 6.812319}, {1.3235601610672, 1.4679465862259});
    model(4, bulk.absorption).mfp = mfp({1e-3, 2e-3, 5e-1}, {1.3, 4.9, 9.4});
    model(4, bulk.rayleigh).mfp = mfp({1e-3, 1e-2}, {3.1, 5.4});
    model(4, bulk.wls).mfp = mfp({1e-3, 2e-3, 5e-1}, {1.3, 4.9, 9.4});
    model(4, bulk.wls2).mfp = mfp({1e-3, 4e-3, 5e-1}, {1.3, 5.9, 8.4});
    model(4, bulk.mie).mfp = mfp({1e-3, 4e-3, 5e-1}, {1.3, 5.9, 8.4});
}
...
auto OpticalMockTestBase::get_mfp_table(ImportModelClass imc) const -> VecGrid
{
    ...
    switch (imc)
    {
        ...
        case ImportModelClass::rayleigh: {
            for (auto&& [id, omm] : bulk.rayleigh.materials)
            {
                CELER_ASSERT(std::holds_alternative<inp::Grid>(omm.mfp));
                result.push_back(std::get<inp::Grid>(omm.mfp));
            }
            break;
        }
