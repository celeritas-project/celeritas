...
class LazySenseCalculatorTest : public ::celeritas::test::OrangeGeoTestBase
{
  protected:
...
TEST_F(LazySenseCalculatorTest, one_volume)
{
    {
        OneVolInput geo_inp;
    ...
    OnFace face;
    LazySenseCalculator calc_senses(this->make_surf_visitor(),
...
TEST_F(LazySenseCalculatorTest, two_volumes)
{
...

    {
        ...
        {
            OnFace face;
            LazySenseCalculator calc_senses(
    ...
    }
    {
        ...
        OnFace face;
        LazySenseCalculator calc_senses(
            this->make_surf_visitor(), inner, Real3{1.5, 0, 0}, face);
        {
            auto result = calc_senses(FaceId{0});
        ...
        {
            face = OnFace{FaceId{0}, Sense::inside};
            auto result = calc_senses(FaceId{0});
            EXPECT_EQ(Sense::inside, result);
            EXPECT_EQ(FaceId{0}, face.id());
...
TEST_F(LazySenseCalculatorTest, five_volumes)
{
