...
TEST(ConeAlignedTest, intersection_typical)
{
    ...
    auto distances = cone.calc_intersections(
...
TEST(ConeAlignedTest, intersection_on_surface)
{
    ...
    auto distances = cone.calc_intersections(
        {1.1 + 3, 2.2 + 2, 3.3}, {0.0, -1.0, 0.0}, SurfaceState::on);
...
TEST(ConeAlignedTest, intersection_along_surface)
{
    ...
    auto distances = cone.calc_intersections(pos, dir, SurfaceState::off);
    ...
    // Inside left sheet
    distances = cone.calc_intersections(
        {1.1 - 3, 2.2 - 2 + 1, 3.3}, dir, SurfaceState::off);
    ...
    distances = cone.calc_intersections(
        {1.1 + 3, 2.2 + 2, 3.3}, dir, SurfaceState::on);
    EXPECT_SOFT_EQ(no_intersection(), distances[0]);
    EXPECT_SOFT_EQ(no_intersection(), distances[1]);
}
...
TEST(ConeAlignedTest, debug_intersect)
{
    ...
    distances = cone.calc_intersections(pos, dir, SurfaceState::on);
...
TEST(ConeAlignedTest, degenerate_boundary)
{
    ...
    for (auto z : {-1.0, 1.0})
    {
        ...
        for (auto eps : {-1e-8, 0.0, 1e-8})
        {
            ...
            auto distances
                = cone.calc_intersections(pos, dir, SurfaceState::on);
