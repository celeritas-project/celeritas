...
class QuarticSolverTest : public ::celeritas::test::Test
{
  public:
    ...
    void expect_surface_roots_from_coeffs(Intersections const& expected,
                                          Coeffs4 const& abcd)
    {
    ...
    void expect_nonsurface_roots_from_coeffs(Intersections const& expected,
                                             Coeffs5 const& abcde)
    {
...
TYPED_TEST(QuarticSolverTest, no_roots)
{
    ...
    {
        this->expect_nonsurface_roots_from_coeffs({no_intersection(),
                                                   no_intersection(),
                                                   no_intersection(),
                                                   no_intersection()},
                                                  {1, 10, 35, 50, 24});
...
TYPED_TEST(QuarticSolverTest, one_root)
{
    ...
    // One root at 2, one negative root at -1, two imag roots
    {
        this->expect_nonsurface_roots_from_coeffs(
            {2.0, no_intersection(), no_intersection(), no_intersection()},
            {1, -3, 1.000001, 2.999999, -2.000002});
    }
}
...
TYPED_TEST(QuarticSolverTest, two_roots)
{
    ...
    {
        this->expect_nonsurface_roots_from_coeffs(
            {1.0, 2.0, no_intersection(), no_intersection()},
            {1, 1, -5, -7, 10});
    }
...
TYPED_TEST(QuarticSolverTest, three_roots)
{
    ...
    {
        this->expect_nonsurface_roots_from_coeffs(
            {1.0, 2.0, 3.0, no_intersection()}, {1, -7, 17, -17, 6});
    }
    // x**4 - 5*x**3 + 5*x**2 + 5*x - 6
    // Three roots at 1, 2, 3, negative root at -1
    {
        this->expect_nonsurface_roots_from_coeffs(
            {1.0, 2.0, 3.0, no_intersection()}, {1, -5, 5, 5, -6});
    }
}
...
TYPED_TEST(QuarticSolverTest, surf_two_roots)
{
    ...
    this->expect_surface_roots_from_coeffs(
        {1.0, 2.0, no_intersection(), no_intersection()}, {1, -3, 2, 0});
}
