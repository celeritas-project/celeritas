...
class ConvexHullFinderTest : public ::celeritas::test::Test
{
  public:
    ...
    void expect_eqs(VecReal2 const& expected, VecReal2 const& actual)
    {
...
TEST_F(ConvexHullFinderTest, basic)
{
    ...
    expect_eqs(VecReal2({p[0], p[1], p[2]}),
               CHF({p[0], p[1], p[2]}).make_convex_hull());
    expect_eqs(VecReal2({p[0], p[1], p[2], p[3]}),
               CHF({p[0], p[1], p[2], p[3]}).make_convex_hull());
    expect_eqs(VecReal2({p[0], p[1], p[2], p[4]}),
               CHF({p[0], p[1], p[2], p[4]}).make_convex_hull());
    expect_eqs(VecReal2({p[0], p[1], p[2], p[4], p[5]}),
               CHF(p).make_convex_hull());

    // Compare concave regions using all 6 points
    expect_eq(VecVecReal2({{p[4], p[3], p[2]}}), CHF(p).calc_concave_regions());
}
//---------------------------------------------------------------------------//
/*
...
TEST_F(ConvexHullFinderTest, first_concavity)
{
    ...
    CHF chf(p);
    expect_eqs(VecReal2({p[0], p[1], p[2], p[4]}), chf.make_convex_hull());
...
TEST_F(ConvexHullFinderTest, colinear)
{
    VecReal2 p{{0, 0},
               ...
               {0.2, -0.4}};
    CHF chf(p);
    expect_eqs(VecReal2({p[0], p[1], p[2], p[3], p[4], p[7], p[8]}),
...
TEST_F(ConvexHullFinderTest, nested_concavity)
{
    ...
    // Test level 0
    CHF chf0(p);
    expect_eqs(VecReal2({p[0], p[1], p[7], p[8], p[9], p[10]}),
    ...
    expect_eqs(VecReal2({p[5], p[4], p[3]}), chf3.make_convex_hull());
