...
function(celeritas_add_test SOURCE_FILE)
    ...
    if(PARSE_UNPARSED_ARGUMENTS)
        message(
            SEND_ERROR
            "Unknown keywords given to celeritas_add_test(): "
    ...
    if(PARSE_INPUTS)
        message(
            FATAL_ERROR
            "INPUTS argument to celeritas_add_test is not implemented"
        )
    endif()
    ...
    foreach(_filter IN LISTS PARSE_FILTER)
        foreach(_np IN LISTS PARSE_NP)
            ...
            set_property(TEST ${_TEST_NAME} PROPERTY ENVIRONMENT ${_test_env})
            if(_np GREATER 1)
                if(PARSE_NT)
                    math(EXPR _np "${_np} * ${PARSE_NT}")
                endif()
                set_property(TEST ${_TEST_NAME} PROPERTY PROCESSORS ${_np})
            endif()
        endforeach()
    endforeach()
    ...
    if(_ADDED_TESTS)
        ...
        set_tests_properties(
            ${_ADDED_TESTS}
            PROPERTIES ${_COMMON_PROPS} "LABELS" "${_LABELS}"
        )
    endif()
