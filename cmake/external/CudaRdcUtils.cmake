...
if(CUDA_RDC_VERSION GREATER _CUDA_RDC_VERSION)
    ...
    message(
        VERBOSE
        "Ignoring CUDA_RDC_VERSION ${_CUDA_RDC_VERSION}: "
...
define_property(
    TARGET
    PROPERTY CUDA_RDC_LIBRARY_TYPE
    BRIEF_DOCS
        "Indicate the type of cuda library (STATIC and SHARED for nvlink usage, FINAL for linking into not cuda library/executable"
    FULL_DOCS
        "Indicate the type of cuda library (STATIC and SHARED for nvlink usage, FINAL for linking into not cuda library/executable"
)
define_property(
    TARGET
    PROPERTY CUDA_RDC_FINAL_LIBRARY
...
define_property(
    TARGET
    PROPERTY CUDA_RDC_STATIC_LIBRARY
...
define_property(
    TARGET
    PROPERTY CUDA_RDC_MIDDLE_LIBRARY
    BRIEF_DOCS
        "Name of the shared (without nvlink step) library corresponding to this cuda library"
    FULL_DOCS
        "Name of the shared (without nvlink step) library corresponding to this cuda library"
)
define_property(
    TARGET
    PROPERTY CUDA_RDC_OBJECT_LIBRARY
    BRIEF_DOCS
        "Name of the object (without nvlink step) library corresponding to this cuda library"
    FULL_DOCS
        "Name of the object (without nvlink step) library corresponding to this cuda library"
)
...
if(CMAKE_SYSTEM_NAME STREQUAL "Linux")
    ...
    set(CMAKE_CXX_LINK_LIBRARY_USING_rdc_no_as_needed_SUPPORTED
        TRUE
        CACHE INTERNAL
        ""
    )
...
macro(cuda_rdc_get_sources_and_options _sources _cmake_options _options)
    ...
    foreach(arg ${ARGN})
        ...
        elseif(
            arg STREQUAL "WIN32"
            OR arg STREQUAL "MACOSX_BUNDLE"
            OR arg STREQUAL "EXCLUDE_FROM_ALL"
            OR arg STREQUAL "STATIC"
            OR arg STREQUAL "SHARED"
            OR arg STREQUAL "MODULE"
            OR arg STREQUAL "ALIAS"
        )
...
function(cuda_rdc_generate_empty_cu_file emptyfilenamevar target)
    set(_stub
        "${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/${target}_emptyfile.cu"
    )
    if(NOT EXISTS ${_stub})
...
function(cuda_rdc_add_library target)
    ...
    if(
        (
            ...
            AND NOT _ADDLIB_PARSE_SHARED
            AND NOT _ADDLIB_PARSE_MODULE
        )
    ...
    if(_ADDLIB_PARSE_OBJECT)
        message(
            FATAL_ERROR
            "cuda_rdc_add_library does not support OBJECT library"
        )
    endif()
    ...
    set(_object_props
        ...
        ${_cudaruntime_requested_type}
    )
    if(_lib_requested_type STREQUAL "SHARED")
        list(APPEND _object_props POSITION_INDEPENDENT_CODE ON)
    endif()
    ...
    set(_common_props
        ...
        LINKER_LANGUAGE
        CUDA
        CUDA_RDC_FINAL_LIBRARY
        ${target}_final
        CUDA_RDC_MIDDLE_LIBRARY
        ${target}
        CUDA_RDC_STATIC_LIBRARY
        ${target}${_staticsuf}
        CUDA_RDC_OBJECT_LIBRARY
        ${target}_objects
    )
    set_target_properties(
        ${target}
        PROPERTIES
            ${_common_props} CUDA_RDC_LIBRARY_TYPE
            Shared CUDA_RESOLVE_DEVICE_SYMBOLS
            OFF # We really don't want nvlink called.
                EXPORT_PROPERTIES
            "CUDA_RUNTIME_LIBRARY;CUDA_RDC_LIBRARY_TYPE;CUDA_RDC_FINAL_LIBRARY;CUDA_RDC_MIDDLE_LIBRARY;CUDA_RDC_STATIC_LIBRARY"
    )
    ...
    set_target_properties(
        ${target}_final
        PROPERTIES
            ${_common_props} LINK_DEPENDS
            $<TARGET_FILE:${target}${_staticsuf}> CUDA_RDC_LIBRARY_TYPE
            Final CUDA_RESOLVE_DEVICE_SYMBOLS
    ...
    foreach(_target_elem IN LISTS _targets)
        ...
        if(NOT _lib_target_type STREQUAL "INTERFACE_LIBRARY")
            ...
            if(_targettype)
                ...
                if(
                    NOT _target_middle STREQUAL _prop_target
                    AND NOT _target_middle STREQUAL _target_static
                )
    ...
    if(TARGET ${lib} AND TARGET ${potentialdepend})
        ...
        if(NOT lib_link_libraries)
            set_property(
                TARGET ${lib}
                APPEND
                PROPERTY CUDA_RDC_NOT_DEPENDS_ON ${potentialdepend}
            )
            return()
        endif()
        foreach(linklib IN LISTS lib_link_libraries)
            ...
            if(linklib STREQUAL potentialdepend)
                set_property(
                    TARGET ${lib}
                    APPEND
                    PROPERTY CUDA_RDC_DEPENDS_ON ${potentialdepend}
                )
                set(${OUTVARNAME} TRUE PARENT_SCOPE)
                return()
            endif()
...
function(cuda_rdc_use_middle_lib_in_property target property)
    ...
    if(_new_values)
        set_target_properties(${target} PROPERTIES ${property} "${_new_values}")
    endif()
endfunction()
...
function(cuda_rdc_find_final_library OUTLIST flat_dependency_list)
    ...
    set_target_properties(
        ${flat_dependcy_list}
        PROPERTIES CUDA_RDC_DEPENDS_ON "" CUDA_RDC_NOT_DEPENDS_ON ""
    )
...
function(cuda_rdc_check_cuda_runtime OUTVAR library)
    ...
    if(NOT _runtime_setting)
        ...
        get_target_property(
            _cuda_middle_library
            ${library}
            CUDA_RDC_MIDDLE_LIBRARY
        )
        get_target_property(
            _cuda_static_library
            ${library}
            CUDA_RDC_STATIC_LIBRARY
        )
        if(_cuda_middle_library AND _cuda_static_library)
            ...
            set_target_properties(
                ${library}
                PROPERTIES CUDA_RUNTIME_LIBRARY "None"
            )
        ...
        elseif(${_final_count} EQUAL 1)
            ...
            if(_final_target_type STREQUAL "STATIC_LIBRARY")
                ...
                foreach(_dep_lib IN LISTS _flat_list_linked_libs)
                    ...
                    if(TARGET ${_alias_dep_lib})
                        ...
                        if(_target_type STREQUAL "STATIC_LIBRARY")
                            list(
                                APPEND
                                _short_flat_list_linked_libs
                                ${_dep_lib}
                            )
                        endif()
                    endif()
                endforeach()
                set(_reverse_flat_list_linked_libs
                    ${_short_flat_list_linked_libs}
                )
                ...
                set_property(
                    ...
                    PROPERTY
                        LINK_LIBRARIES
                            ${_reverse_flat_list_linked_libs}
                            ${_short_flat_list_linked_libs}
                            ${_finallibs}
                            ${_current_link_libraries}
                ...
                set_property(
                    ...
                    PROPERTY
                        LINK_LIBRARIES
                            ${_current_link_libraries}
                            "$<LINK_LIBRARY:rdc_no_as_needed,${_finallibs}>"
                            ${_cuda_runtime_target}
                )
            endif()
    ...
    if(_contains_cuda)
        ...
        foreach(_lib IN LISTS _flat_target_link_libraries)
            ...
            if(NOT _lib_target_type STREQUAL "INTERFACE_LIBRARY")
                ...
                if(
                    NOT _need_to_use_shared_runtime
                    AND _lib_runtime_setting STREQUAL "Shared"
                )
                    set(_need_to_use_shared_runtime TRUE)
                endif()
                if(
                    NOT _first_lib_runtime_setting
                    ...
                    if(
                        NOT _current_runtime_setting
                        OR NOT _current_runtime_setting
                            STREQUAL
                            _first_lib_runtime_setting
                    )
                        set_target_properties(
                            ${target}
                            PROPERTIES
                                CUDA_RUNTIME_LIBRARY ${_lib_runtime_setting}
                        )
                        set(_current_runtime_setting ${_lib_runtime_setting})
                    endif()
                elseif(
                    _lib_runtime_setting
                    AND NOT (_first_lib_runtime_setting STREQUAL _lib_runtime_setting)
                    AND NOT _lib_runtime_setting STREQUAL "None"
                )
                    ...
                    message(
                        FATAL_ERROR
                        "The CUDA runtime used for ${_lib} [${_lib_runtime_setting}] is different from the one used by ${_first_lib}  [${_first_lib_runtime_setting}]"
                    )
                endif()
                ...
                if(TARGET ${_libstatic})
                    target_link_options(
                        ${_target_final}
                        PRIVATE $<DEVICE_LINK:$<TARGET_FILE:${_libstatic}>>
                    )
                    set_property(
                        TARGET ${_target_final}
                        APPEND
        ...
        if(_need_to_use_shared_runtime)
            ...
            set_property(
                TARGET ${_target_final}
                APPEND
                PROPERTY LINK_LIBRARIES ${CMAKE_DL_LIBS}
            )
        endif()
...
function(cuda_rdc_target_sources target)
    ...
    if(_contains_cuda)
        ...
        if(_target_type STREQUAL "EXECUTABLE")
            ...
            message(
                FATAL_ERROR
                "The target ${target} is a ${_target_type} and we can not add"
...
function(cuda_rdc_cuda_gather_dependencies outlist target)
    ...
    get_target_property(
        ...
        ${target}
        CUDA_RDC_CACHED_LIB_DEPENDENCIES
    )
    ...
    if(NOT _target_type STREQUAL "INTERFACE_LIBRARY")
        ...
        get_target_property(
            _target_interface_link_libraries
            ${target}
            INTERFACE_LINK_LIBRARIES
        )
    ...
    set_target_properties(
        ${target}
        PROPERTIES CUDA_RDC_CACHED_LIB_DEPENDENCIES "${_deplist}"
    )
