...
#-----------------------------------------------------------------------------#
# Return the static library of the target, if any.
macro(cuda_rdc_get_library_static_target outvar target)
  get_target_property(_target_type ${target} TYPE)
  if(NOT _target_type STREQUAL "INTERFACE_LIBRARY")
    get_target_property(${outvar} ${target} CUDA_RDC_STATIC_LIBRARY)
  else()
    set(${outvar} ${target})
  endif()
endmacro()
...
function(cuda_rdc_use_middle_lib_in_property target property)
  ...
  foreach(_lib ${_target_libs})
    ...
    if(TARGET ${_lib})
      ...
      if(_libmid)
        # We need to see if any of the dependent libraries are
        # middle library that are static, we need to list those
        # explicitly here.

        set(_flat_midlist)
        cuda_rdc_cuda_gather_dependencies(_flat_midlist ${_libmid})
        foreach(_sublib ${_flat_midlist})
          get_target_property(_sublib_target_type ${_sublib} TYPE)
          if(_sublib_target_type STREQUAL "STATIC_LIBRARY")
            if(_stripped_lib)
              list(APPEND _new_values "$<LINK_ONLY:${_sublib}>")
            else()
              list(APPEND _new_values ${_sublib})
            endif()
          endif()
        endforeach()
        # Remove this
        if(_target_type STREQUAL "STATIC_LIBRARY")
          cuda_rdc_get_library_static_target(_staticlib ${_lib})
          if(_staticlib)
            set(_libmid ${_staticlib})
          endif()
        endif()
        if(_stripped_lib)
  ...
  if(NOT _target_type STREQUAL "INTERFACE_LIBRARY")
    ...
    if(_target_link_libraries)
      foreach(_lib ${_target_link_libraries})
        ...
        if(_target_type STREQUAL "STATIC_LIBRARY")
          foreach(_subelem ${_midlist})
            cuda_rdc_get_library_static_target(_staticlib ${_subelem})
            if(_staticlib)
               list(APPEND _deplist ${_staticlib})
            else()
               list(APPEND _deplist ${_subelem})
            endif()
          endforeach()
        else()
