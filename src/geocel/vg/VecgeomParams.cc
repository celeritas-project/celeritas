...
VecgeomParams::from_gdml_g4(std::string const& filename)
{
    CELER_VALIDATE(celeritas::global_geant_geo().expired(),
                   << "cannot load Geant4 geometry into VecGeom from a file name: a global Geant4 geometry already exists");
...
{
    CELER_EXPECT(geo);
    CELER_LOG(status) << "Loading VecGeom geometry from in-memory Geant4 geometry";
#if CELERITAS_USE_GEANT4
