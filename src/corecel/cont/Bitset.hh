...
class Bitset
{
  ...
  public:
    ...
    CELER_CONSTEXPR_FUNCTION bool operator[](size_t pos) const noexcept
    {
        return (get_word(pos) & mask(pos)) != static_cast<word_type>(0);
    }

    CELER_CONSTEXPR_FUNCTION reference operator[](size_t pos) noexcept
    {
        return reference(*this, pos);
    }

    CELER_CONSTEXPR_FUNCTION bool test(size_t pos) const
    ...
    CELER_CONSTEXPR_FUNCTION bool none() const noexcept { return !any(); }

    CELER_CONSTEXPR_FUNCTION size_t count() const noexcept
    {
        size_t count = 0;
        for (size_t i = 0; i < num_words; ++i)
        {
    ...
    CELER_CONSTEXPR_FUNCTION size_t size() const noexcept { return N; }
    ...
    CELER_CONSTEXPR_FUNCTION Bitset& set(size_t pos, bool value = true) noexcept
    {
        if (value)
        {
            get_word(pos) |= mask(pos);
        }
        else
        {
            get_word(pos) &= ~mask(pos);
    ...
    CELER_CONSTEXPR_FUNCTION Bitset& reset(size_t pos) noexcept
    {
        get_word(pos) &= ~mask(pos);
        return *this;
    }
    ...
    CELER_CONSTEXPR_FUNCTION Bitset& flip(size_t pos) noexcept
    {
        get_word(pos) ^= mask(pos);
        return *this;
    }
  ...
  private:
    static CELER_CONSTEXPR_FUNCTION size_t which_word(size_t pos) noexcept
    {
        return pos / bits_per_word;
    }

    static CELER_CONSTEXPR_FUNCTION size_t which_bit(size_t pos) noexcept
    {
        return pos % bits_per_word;
    }

    static CELER_CONSTEXPR_FUNCTION word_type mask(size_t pos) noexcept
    {
        return static_cast<word_type>(1) << which_bit(pos);
    }

    CELER_CONSTEXPR_FUNCTION word_type get_word(size_t pos) const noexcept
    {
        return words_[which_word(pos)];
    }

    CELER_CONSTEXPR_FUNCTION word_type& get_word(size_t pos) noexcept
    {
        return words_[which_word(pos)];
    }
