 ...
 */
template<MemSpace M>
inline void extend_from_primaries(CoreRef<M> const& core_data,
                                  Span<Primary const> host_primaries)
{
    CELER_EXPECT(core_data);
    CELER_EXPECT(!host_primaries.empty());

    auto& data = core_data.states.init.initializers;
 ...
 */
template<MemSpace M>
inline void initialize_tracks(CoreRef<M> const& core_data)
{
    CELER_EXPECT(core_data);
    ...
    if (num_tracks > 0)
    {
        ...
        const_cast<NonConstT&>(data).initializers.resize(
            data.initializers.size() - num_tracks);
        const_cast<NonConstT&>(data).vacancies.resize(data.vacancies.size()
                                                      - num_tracks);
    }
}
 ...
 */
template<MemSpace M>
inline void extend_from_secondaries(CoreRef<M> const& core_data)
{
    CELER_EXPECT(core_data);
    ...
    const_cast<NonConstT&>(data).vacancies.resize(core_data.states.size());
    ...
    size_type num_vac = detail::remove_if_alive<M>(
        const_cast<NonConstT&>(data).vacancies.data());
    const_cast<NonConstT&>(data).vacancies.resize(num_vac);
    ...
    // interaction.
    const_cast<NonConstT&>(data).num_secondaries
    ...
    const_cast<NonConstT&>(data).initializers.resize(data.initializers.size()
                                                     + data.num_secondaries);
