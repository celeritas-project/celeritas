...
class FieldSubstepper
{
  public:
    ...
    inline CELER_FUNCTION Substep accurate_advance(
  ...
  private:
    ...
    inline CELER_FUNCTION ChordSearch find_next_chord(
        real_type step, OdeState const& state) const;

    // Advance for a given step and evaluate the next predicted step.
    inline CELER_FUNCTION Integration integrate_step(
        real_type step, OdeState const& state) const;

    // Advance within the truncated error and estimate a good next step size
    inline CELER_FUNCTION Integration one_good_step(
        real_type step, OdeState const& state) const;
 ...
 */
template<class IntegratorT>
CELER_FUNCTION FieldSubstepper<IntegratorT>::FieldSubstepper(
    FieldDriverOptions const& options, IntegratorT&& integrate)
    : options_(options)
 ...
 */
template<class IntegratorT>
CELER_FUNCTION Substep FieldSubstepper<IntegratorT>::operator()(
    real_type step, OdeState const& state)
{
 ...
 */
template<class IntegratorT>
CELER_FUNCTION real_type FieldSubstepper<IntegratorT>::new_step_scale(
    real_type err_sq) const
{
