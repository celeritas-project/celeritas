 ...
 * of a cubic spline (see: \sa SplineInterpolator) requires solving a
 * tridiagonal, linear system of equations for the second derivatives. For \f$
 * n \f$ points \f$ (x_i, y_i) \f$ and \f$ n \f$ unknowns \f$ y''_i \f$ there
 * are \f$ n - 2 \f$ equations of the form
 * \f[
   h_{i - 1} y''_{i - 1} + 2 (h_{i - 1} + h_i) y''i + h_i y''_{i + 1} = 6 r_i,
 * \f]
 ...
 * Natural boundary conditions set \f$ y''_0 = y''_{n - 1} = 0 \f$, which leads
 * to the following initial and final equations:
 * \f{align}{
   2 (h_0 + h_1) y''_1 + h_1 y''_2 &= 6 r_1 //
   h_{n - 3} y''_{n - 3} + 2 (h_{n - 3} + h_{n - 2}) y''_{n - 2}
   &= 6 r_{n - 2}.
 * \f}
 *
 ...
 * require the third derivative \f$ y'''_i \f$ to be continous across the first
 ...
 * \f{align}{
   \frac{(h_0 + 2 h_1)(h_0 + h_1)}{h_1} y''_1 + \frac{h_1^2 - h_0^2}{h_1}
   y''_2 &= 6 r_1 //
   \frac{h_{n - 3}^2 - h_{n - 2}^2}{h_{n - 3}} y''_{n - 3} + \frac{(h_{n - 3}
   + h_{n - 2})(2 h_{n - 3} + h_{n - 2})}{h_{n - 3}} y''_{n - 2} &= 6 r_{n - 2}
 * \f}
 * Once the system of equations has been solved for the second derivatives, the
 * derivatives \f$ y''_0 \f$ and \f$ y''_{n - 1} \f$ can be recovered:
 * \f{align}{
   y''_0 &= \frac{(h_0 + h_1) y''_1 - h_0 y''_2}{h_1} \\
   y''_{n - 1} &= \frac{(h_{n - 3} + h_{n - 2}) y''_{n - 2} - h_{n - 2}
   y''_{n - 3}}{h_{n - 3}}
 * \f}
 */
