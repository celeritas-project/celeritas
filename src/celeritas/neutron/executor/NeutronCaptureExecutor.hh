//------------------------------- -*- C++ -*- -------------------------------//
// Copyright Celeritas contributors: see top-level COPYRIGHT file for details
// SPDX-License-Identifier: (Apache-2.0 OR MIT)
//---------------------------------------------------------------------------//
//! \file celeritas/neutron/executor/NeutronCaptureExecutor.hh
//---------------------------------------------------------------------------//
#pragma once

#include "corecel/Assert.hh"
#include "celeritas/global/CoreTrackView.hh"
#include "celeritas/mat/ElementView.hh"
#include "celeritas/neutron/data/NeutronCaptureData.hh"
#include "celeritas/neutron/interactor/NeutronCaptureInteractor.hh"
#include "celeritas/neutron/xs/NeutronCaptureMicroXsCalculator.hh"
#include "celeritas/phys/Interaction.hh"
#include "celeritas/random/ElementSelector.hh"

namespace celeritas
{
//---------------------------------------------------------------------------//
struct NeutronCaptureExecutor
{
    inline CELER_FUNCTION Interaction
    operator()(celeritas::CoreTrackView const& track);

    NativeCRef<NeutronCaptureData> params;
};

//---------------------------------------------------------------------------//
/*!
 * Apply the NeutronCaptureInteractor to the current track.
 */
CELER_FUNCTION Interaction
NeutronCaptureExecutor::operator()(CoreTrackView const& track)
{
    auto particle = track.particle();

    // Select a target element
    auto material = track.material().material_record();
    auto elcomp_id = track.physics_step().element();
    if (!elcomp_id)
    {
        // Sample an element (based on element cross sections on the fly)
        auto rng = track.rng();
        ElementSelector select_el(
            material,
            NeutronCaptureMicroXsCalculator{params, particle.energy()},
            track.material().element_scratch());
        elcomp_id = select_el(rng);
        CELER_ASSERT(elcomp_id);
        track.physics_step().element(elcomp_id);
    }

    // Construct the interactor
    NeutronCaptureInteractor interact(params, particle);

    // Execute the interactor
    return interact();
}

//---------------------------------------------------------------------------//
}  // namespace celeritas
