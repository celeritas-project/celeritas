    ...
    : G4VEnergyLossProcess("eBrem"), model_selection_(selection)
{
    CELER_VALIDATE(selection != ModelSelection::none,
                   << "Cannot initialize GeantBremsstrahlungProcess with BremsModelSelection::none.");
    ...
    {
        CELER_LOG(warning)
            << "Bremmstrahlung without a model at low energies is not supported: extending relativistic model down to "
            << energy_min << " MeV";
    ...
    {
        CELER_LOG(warning)
            << "Using bremmstrahlung without a relativistic model may result in failures for high energy tracks";
    }
