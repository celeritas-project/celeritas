...
{
//---------------------------------------------------------------------------//
IOPAContextException::IOPAContextException(ImportOpticalProcessClass ipc,
                                           MaterialId mid)
{
    std::stringstream os;
...
std::shared_ptr<ImportOpticalProcesses>
ImportOpticalProcesses::from_import(ImportData const& data)
{
    ImportOpticalProcess absorption;
    absorption.process_class = ImportOpticalProcessClass::absorption;
    absorption.lambda_table.table_type = ImportTableType::lambda;

    ImportOpticalProcess rayleigh;
    rayleigh.process_class = ImportOpticalProcessClass::rayleigh;
    rayleigh.lambda_table.table_type = ImportTableType::lambda;

    for (auto mat_idx : range(ImportData::MatIdx{data.materials.size()}))
    {
        // ImportMaterial const& material = data.materials[mat_idx];
        ImportOpticalMaterial const& optical_material
            = data.optical.at(mat_idx);

        absorption.lambda_table.physics_vectors.push_back(
            optical_material.absorption.absorption_length);
        rayleigh.lambda_table.physics_vectors.push_back(
            optical_material.rayleigh.mfp);
    }

    return std::make_shared<ImportOpticalProcesses>(
        std::vector<ImportOpticalProcess>{std::move(absorption),
                                          std::move(rayleigh)});
}

ImportOpticalProcesses::ImportOpticalProcesses(
    std::vector<ImportOpticalProcess> io)
...
auto ImportOpticalProcesses::find(key_type process) const
    -> ImportOpticalProcessId
{
...
ImportedOpticalProcessAdapter::ImportedOpticalProcessAdapter(
    SPConstImported imported, ImportOpticalProcessClass process_class)
    ...
    for (auto material : range(MaterialId{mats.size()}))
    {
        ...
        catch (...)
        {
            std::throw_with_nested(
                IOPAContextException(process_class_, material));
        }
    }
