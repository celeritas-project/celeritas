...
auto MatScintSpecInserter::operator()(ImportMaterialScintSpectrum const& mat)
{
    ...
    if (mat)
    {
        // Only if the material has the scintillation spectrum
        ...
        spectrum.yield_pdf
            = reals_.insert_back(yield_pdf.begin(), yield_pdf.end());

        CELER_LOG(debug) << "mat spectrum is : " << spectrum.components.size();
        CELER_ENSURE(spectrum.components.size() == mat.components.size());
    }
    else
    {
        // No scintillation was found for the material.
        // setting it to zero
        spectrum.yield_per_energy = 0.0;
        spectrum.components
            = {scint_records_.size_id(), scint_records_.size_id()};
        spectrum.yield_pdf = {};
        CELER_LOG(debug)
            << "No scintillation spectrum found for the material - setting "
               "the spectrum to "
            << spectrum.yield_per_energy;
    }
