    ...
    : volumes_{std::move(volumes)}
{
    ...
    host_data.scalars.num_univ_levels = [&univ_inp = input.universes] {
        auto result = detail::DepthCalculator{univ_inp}();
        constexpr auto max_ulev
            = static_cast<int>(UnivLevelId{}.unchecked_get()) - 1;
        CELER_VALIDATE(result <= max_ulev,
                       << "universe level " << result
                       << " exceeds maximum hardcoded depth " << max_ulev);
        return static_cast<UnivLevelId::size_type>(result);
    }();
