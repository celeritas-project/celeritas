    ...
    -> Initialization
{
    ...
    for (auto ax : range(Axis::size_))
    {
        ...
        VolumeIndexer to_index(
            {record_.dims[2], record_.dims[1], record_.dims[0]});
     ...
     * Calculate distance-to-intercept for the next surface.
     */
    CELER_FUNCTION auto RectArrayTracker::intersect(LocalState const& state)
        const->Intersection
    {
     ...
     *
     * On an axis-aligned rectlinear grid the minimum distance to any surface
     * is always always occurs along a line parallel to an axis.
     */
    CELER_FUNCTION real_type RectArrayTracker::safety(
        Real3 const& pos, LocalVolumeId volid) const
    {
    ...
    CELER_FUNCTION autouniv
        / RectArrayTracker
              .hh
              RectArrayTracker::normal(Real3 const&, LocalSurfaceId surf)
                  const->Real3
    {
     ...
     */
    template<class F>
    CELER_FUNCTION auto RectArrayTracker::intersect_impl(
        LocalState const& state, F is_valid) const->Intersection
    {
        ...
        for (auto ax : range(Axis::size_))
        {
            ...
            real_type dist = (target_value
                              - static_cast<real_type>(state.pos[to_int(ax)]))
                             / state.dir[to_int(ax)];
            ...
            if (dist > 0 && is_valid(dist) && dist < result.distance)
            {
                ...
                auto local_surface = LocalSurfaceId(to_index(
     ...
     * Create Grid object for a given axis.
     */
    CELER_FUNCTION RectArrayTracker::Grid RectArrayTracker::make_grid(Axis ax)
        const
    {
