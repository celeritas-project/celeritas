...
CELER_FUNCTION auto RungeKuttaStepper<T>::
                    operator()(real_type step, const OdeArray& beg_state, const OdeArray& beg_slope)
    -> OdeArray
{
    ...
    OdeArray end_state = stepper(half_step, mid_state_, equation_(mid_state_));

    // Do a full step
    OdeArray yt = stepper(step, beg_state, beg_slope);

    // Stepper error: difference between the full step and two half steps
    stepper_error_ = end_state;
    axpy(real_type(-1.0), yt, &stepper_error_);

    // Output correction with the 4th order coefficient (1/15)
    axpy(fourth_order_correction, stepper_error_, &end_state);
    ...
    -> OdeArray
{
    ...
    OdeArray mid_est = beg_state;
    axpy(half_step, beg_slope, &mid_est);
    OdeArray mid_est_slope = equation_(mid_est);

    // 2nd step k2 = (step/2)*mid_est_slope
    OdeArray mid_state = beg_state;
    axpy(half_step, mid_est_slope, &mid_state);
    OdeArray mid_slope = equation_(mid_state);

    // 3rd step k3 = step*mid_slope
    OdeArray end_est = beg_state;
    axpy(step, mid_slope, &end_est);
    OdeArray end_slope = equation_(end_est);
                                                     ...
                                                     const OdeArray& beg_state)
{
    ...
    for (auto i : range(3))
    {
        ...
        errpos2 += stepper_error_[i] * stepper_error_[i];
        errvel2 += stepper_error_[i + 3] * stepper_error_[i + 3];
    }
...
CELER_FUNCTION real_type RungeKuttaStepper<T>::distance_chord(
    const OdeArray& beg_state, const OdeArray& end_state) const
{
