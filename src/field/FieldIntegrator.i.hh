 ...
 */
CELER_FUNCTION 
real_type FieldIntegrator::advance_chord_limited(real_type  hstep,
                                                 ode_type&  y)       
{
    ode_type yend = y;
                                           ...
                                           real_type&      dyerr)
{
    ...
    { 
        ...
        if(dchord_step <= shared_.delta_chord) break;
                                                        ...
                                                        real_type& hnext)
{
    ...
    for (CELER_MAYBE_UNUSED int i : celeritas::range(shared_.max_nsteps))
    {
        ...
        if ( errmax2 <= 1.0 )  { break; } // step succeeded. 

        // Step failed; compute the size of retrial step.
        real_type htemp =
             shared_.safety*h*std::pow(errmax2, 0.5*shared_.pshrink);
...
CELER_FUNCTION bool 
FieldIntegrator::accurate_advance(real_type  hstep,
                                  ode_type&  y,       
...
CELER_FUNCTION
real_type FieldIntegrator::new_step_size(real_type  hstep, 
		                         real_type  error) 
{
    CELER_ASSERT(error > 0);

    // Estimate the new trial step for a failed step
    real_type scale_factor = (error > 1.0) ? 
        std::pow(error,shared_.pshrink) : std::pow(error,shared_.pgrow);
