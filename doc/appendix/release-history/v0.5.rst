...
* Add optical "locate vacancies" and pre-step actions *(@amandalund, #1441, 4995dd5e)*
* Extend debug utilities for stuck/errored tracks *(@sethrj, #1451, 5c15b548)*
* Add CELER_NONFATAL_FLUSH to cleanly kill stuck tracks *(@sethrj, #1455, dbb446a8)*
* Add optical absorption model with imported data *(@hhollenb, #1440, bde923f9)*
* Improve error checking and diagnostic output for managing GPU *(@sethrj, #1464, 19ff6efc)*
* Override and save Geant4 dataset variables at configure time *(@pcanal, #1475, 1be88a44)*
* Add multilevel "volume instance"  *(@sethrj, #1461, 1b6b8b23)*
* Reconstruct Geant4 navigation history from stack *(@sethrj, #1466, e0e3c007)*
* Add multi-level state to detector step result *(@sethrj, #1471, 1c80d26c)*
* Reconstruct "touchable" using navigation hierarchy *(@sethrj, #1495, f857f6d5)*
* Add "cut" for individual track steps *(@sethrj, #1467, 9c599f98)*
* Default SD integration to "true" *(@sethrj, #1501, e390d559)*
* Add support for Geant4 navigator through offload interface *(@sethrj, #1558, 96d4e589)*

Reviewers: @amandalund *(6)*, @pcanal *(4)*, @sethrj *(2)*, @mrguilima *(2)*, @whokion *(1)*, @stognini *(1)*

Bug fixes
---------

* Define a "unique event ID" different from event counter *(@sethrj, #1447, 51f7e379)*
* Fix optical import when unused materials are defined *(@sethrj, #1449, 9e8e3324)*
* Remove trackid counter from primary *(@sethrj, #1448, 0865304b)*
* Improve single-event mode in ``accel`` *(@sethrj, #1452, 86152b94)*
* Fix GDML export messages and geometry-only use case *(@sethrj, #1454, 6ec65e75)*
* Fix tracking cut message *(@sethrj, #1458, 16f00aea)*
* Fix hepmc3/macro initialization in celer-g4 *(@sethrj, #1535, cbf14de9)*
* Fix linking Athena with VecGeom+CUDA *(@pcanal, #1489, fdd91420)*
* Fix unnecessary linking when using VecGeom+CUDA *(@pcanal, #1487, fc4eb10e)*
* Use canonical loop form with OpenMP *(@amandalund, #1492, cd099cbd)*
* Anchor vtables to fix broken dynamic cast in tests *(@sethrj, #1474, 5a0178e7)*
* Check VecGeom BVH device pointers at setup and run time *(@sethrj, #1481, f20c0826)*
* Fix celer-sim MC truth ROOT output file generation during tests *(@stognini, #1515, 6b053cd2)*
...
* Improve XSCalculator documentation and tests *(@sethrj, #1450, 24e7a0fa)*
* Add CITATION file with CHEP 2024 article *(@sethrj, #1529, 8e11b637)*
* Update Readme and Contributing doc *(@pcanal, #1532, bd300437)*
* Add missing shell setup in README *(@pcanal, #1545, 442b9ac7)*
* Fix/standardize multi-level.gdml file and improve debug logging *(@sethrj, #1493, 00f4495f)*
* Add debugging information for app tests *(@sethrj, #1504, f8b2d56e)*
* Hotfix: Disable ROCm due to now-unsupported CI OS image *(@sethrj, #1521, ca6099b6)*
* Update post-PR workflow to ``actions/download-artifact@v4`` *(@esseivaju, #1523, 9fb13d4c)*
* Trim export data for unit tests *(@sethrj, #1527, f8a3259a)*
* Fix accel examples and related CI issues *(@sethrj, #1557, 514dadcf)*
* Update docker and Spack CI *(@sethrj, #1555, 85952d2b)*
* Update CMake policy *(@sethrj, #1563, 1b974d33)*
* Update expected safeties in Vecgeom tests and CMake version range *(@mrguilima, #1552, 56a2ea58)*
* Prepare to update copyrights *(@sethrj, #1559, 2cce92ec)*

Reviewers: @amandalund *(5)*, @pcanal *(5)*, @sethrj *(3)*, @esseivaju *(2)*, @mrguilima *(1)*

Minor internal changes
----------------------

* Give priority to existing environment variables for G4 data location *(@pcanal, #1457, fe52da7e)*
* Define ``id_cast`` helper function *(@sethrj, #1459, 7038ea4a)*
* Simplify geometry "volume" interface *(@sethrj, #1460, aab88dd1)*
* Use kernel launchers instead of macros *(@sethrj, #1477, 4dfb22e8)*
* Escape semicolons in exported CMake variables *(@drbenmorgan, #1472, 3a4ddef1)*
* Require ``CELERITAS_DEBUG`` be on to enable ``CELERITAS_DEVICE_DEBUG`` *(@amandalund, #1476, 50456f6a)*
* Fix missing brace and refactor #1475 *(@sethrj, #1480, efd4b7c1)*
* Split CeleritasUtils cmake and use VecGeom RDC if available *(@sethrj, #1514, aeecb15e)*
* Refactor step storage to use aux state *(@sethrj, #1465, 0c8c48d6)*
* Refactor "touchable updater" that uses a navigator *(@sethrj, #1483, f07cc633)*
* Fix dBVH memcpy size *(@esseivaju, #1490, 3e0a2bea)*
* Refactor pinned allocator to reduce need for templates *(@sethrj, #1468, 03e90b2c)*
* Tweak Perfetto integration *(@sethrj, #1470, 8a3bfb53)*
* Fix build with latest Geant4 develop *(@pcanal, #1484, 3c8ed961)*
* Improve logger when using either CUDA or MPI *(@sethrj, #1482, a25833f5)*
* Work around RDC-related link errors when +cuda+vecgeom~shared *(@esseivaju, #1498, 8d52d45c)*
* Fix redundant move warning *(@esseivaju, #1500, 51ed71e7)*
* Stop CMake from cleaning root test data *(@hhollenb, #1516, ffb8fee7)*
* Move ``alongstep`` directory to top level *(@sethrj, #1499, 9879c714)*

Reviewers: @sethrj *(7)*, @amandalund *(6)*, @pcanal *(5)*, @esseivaju *(2)*

Deprecation and removal
-----------------------

* Remove curand performance test *(@sethrj, #1543, 47258452)*

Reviewers: @amandalund *(1)*

**Full Changelog**: https://github.com/celeritas-project/celeritas/compare/vbackports/v0.5...v0.5.1
