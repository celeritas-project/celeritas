...
- Sensitive detectors callbacks from Celeritas can bebe disabled with a new
  macro flag
- Geant4 10.5 is now supported
- A new step diagnostic in ``celer-g4`` prints step distributions from Geant4
  CPU execution

Key fixes:

- Backward compatibility with Geant4 10.6 is fixed
- Support more than one MSC model being defined in Geant4
- Initializer capacity in ``celer-sim`` is nos scaled to be per thread
