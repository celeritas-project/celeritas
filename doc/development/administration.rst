...
- Contributor as "Editor": reviewers of PRs, ordered by number reviews
...
- Reusing slides from previous public presentations should contain small text
  (I usually use thin, ultra-light italics in gray) with a citation of the
...
- Direct contributors to the article, using roles defined by `CRediT`_
  (Contributor Roles Taxonomy), get authorship credit.
...
- Mentoring, coordinating, funding acquisition, etc. are not sufficient in
  themselves to be marked as authors. However, these activities should motivate
  direct participation (editing, revising) as authors.

.. note:: If possible, make the Celeritas release used in the article the
   *first* reference in the docment.
