...
# ---------------------------------------------------------------------------
# main(): --types filtering
# ---------------------------------------------------------------------------


def test_main_types_processes_matching_extension(tmp_path):
    p = write(tmp_path, "f.cc", "arrow\u2192right")
    assert main(["--types", "cc", str(p)]) == 1
    assert p.read_text() == "arrow->right"


def test_main_types_skips_non_matching_extension(tmp_path):
    original = "arrow\u2192right"
    p = write(tmp_path, "f.py", original)
    assert main(["--types", "cc", str(p)]) == 0
    assert p.read_text() == original  # untouched


def test_main_types_handles_cmakelists(tmp_path):
    """CMakeLists.txt is matched when 'cmake' is in --types."""
    p = write(tmp_path, "CMakeLists.txt", "# \u2026 comment")
    assert main(["--types", "cmake", str(p)]) == 1
    assert p.read_text() == "# ... comment"


def test_main_types_skips_cmakelists_without_cmake(tmp_path):
    """CMakeLists.txt is skipped when 'cmake' is not in --types."""
    original = "# \u2026 comment"
    p = write(tmp_path, "CMakeLists.txt", original)
    assert main(["--types", "cc,hh", str(p)]) == 0
    assert p.read_text() == original


def test_main_types_multiple_extensions(tmp_path):
    """Multiple extensions can be specified together."""
    cc = write(tmp_path, "f.cc", "a\u2014b")
    hh = write(tmp_path, "f.hh", "x\u2013y")
    py = write(tmp_path, "f.py", "z\u2026w")
    result = main(["--types", "cc,hh", str(cc), str(hh), str(py)])
    assert result == 1
    assert cc.read_text() == "a--b"
    assert hh.read_text() == "x-y"
    assert py.read_text() == "z\u2026w"  # skipped


def test_main_types_leading_dot_ignored(tmp_path):
    """Extension specified as '.cc' is treated the same as 'cc'."""
    p = write(tmp_path, "f.cc", "a\u2192b")
    assert main(["--types", ".cc", str(p)]) == 1
